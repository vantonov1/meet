...
interface EquityPriceRangeRepository : ReactiveCrudRepository<PriceRange, Long> {
    @Query("select min(price) as minPrice, max(price) as maxPrice from equity where type in (:type) and hidden is null ")
    fun getPriceRange(type: List<Int>): Mono<PriceRange>

    @Query("select min(price), max(price) from equity where type in (:type) and hidden is null " +
            " and district in (:district) and subway in (:subway)")
    fun getPriceRangeWithDistrictAndSubway(type: List<Int>, district: List<Byte>, subway: List<String>): Mono<PriceRange>

    @Query("select min(price), max(price) from equity where type in (:type) and hidden is null " +
            " and subway in (:subway)")
    fun getPriceRangeWithSubway(type: List<Int>, subway: List<String>): Mono<PriceRange>

    @Query("select min(price), max(price) from equity where type in (:type) and hidden is null " +
            " and district in (:district)")
    fun getPriceRangeWithDistrict(type: List<Int>, district: List<Byte>): Mono<PriceRange>
}
